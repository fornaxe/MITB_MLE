...
def process_labels_gold_table(snapshot_date_str, silver_lms_dir, gold_label_dir, spark, dpd=30, mob=6):
    ...
    Also carries loan_start_date so downstream joins can match features at
    origination (loan_start_date) rather than at label observation (snapshot_date).
    Features are captured at loan origination; labels are observed mob months later.
    """
    ...
    df = df.withColumn("loan_start_date", col("loan_start_date").cast(StringType()))
    df = df.select("loan_id", "Customer_ID", "label", "label_def", "snapshot_date", "loan_start_date")
